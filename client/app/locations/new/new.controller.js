...
angular.module('treasuremapApp')
  ...
  .controller('UploadCtrl', function ($scope, Upload) {
    ...
    var S3_SECRET = 'OAiEQA8MY7o+kGPhYIB9M4W1tqKQokTun2xZehsg';
    var S3_KEY = 'AKIAJTIPCXDWAFSGFM3A';
    var S3_POLICY = {
    ...
    $scope.upload = function (files) {
      if (files && files.length) {
        for (var i = 0; i < files.length; i++) {
          var file = files[i];
          Upload.upload({
            url: 'https://treasure-map.s3.eu-central-1.amazonaws.com',
            method: 'POST',
            fields : {
              key: file.name,
              AWSAccessKeyId: S3_KEY,
              acl: 'private',
              policy: window.btoa( result ),
              signature: window.btoa( S3_SIGNATURE ),
